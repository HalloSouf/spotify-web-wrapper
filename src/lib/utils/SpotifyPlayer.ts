...
class SpotifyPlayer extends EventEmitter {
	...
	private patch(): void {
		...
		this.client.addListener('player_state_changed', (payload) =>
			this.emit('player_state_changed', payload)
		);
		this.client.addListener('account_error', ({ message }) =>
			this.emit('error', { type: 'account_error', message })
		);
		this.client.addListener('authentication_error', ({ message }) =>
			this.emit('error', { type: 'authentication_error', message })
		);
		this.client.addListener('initialization_error', ({ message }) =>
			this.emit('error', { type: 'initialization_error', message })
		);
		this.client.addListener('playback_error', ({ message }) =>
